...
//https://gist.github.com/carolynvs/3344800#file-autoconverttoutcdatacontext-cs
        ...
        public static void Convert(object model)
        {
            ...
            if (!_typesWithDateTimeProperties.ContainsKey(modelType))
            {
                lock (_lock)
                {
                    if (!_typesWithDateTimeProperties.ContainsKey(modelType))
                    {
                        dateTimeProperties = modelType.GetProperties().Where(p => p.PropertyType.Name == "DateTime");
